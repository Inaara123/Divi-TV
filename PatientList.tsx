...
const PatientList = () => {
    ...
    const [pratCurrent, setPratCurrent] = useState(0);

    const itemsPerPage = 10;

    useEffect(() => {
        const dbRef = ref(database, 'reception/patient');
        const rootData = ref(database, '/');
        const fetchPatients = () => {
            onValue(dbRef, (snapshot) => {
                const data = snapshot.val();
                if (data) {
                    const list = Object.keys(data).filter(key => !data[key].deleted).map(key => ({
                        id: key,
                        ...data[key]
                    }));
                    ...
                    setDisplayedPatients(uniqueList.slice(start, end));
                } else {
                    setAllPatients([]);
        ...
        const fetchRootData = () => {
            onValue(rootData, (snapshot) => {
                ...
                if (rootData && ( rootData.pratcurrentcount) !== undefined) {
                    setPratCurrent(rootData.pratcurrentcount);
                }
            });
        };
    ...
    const renderItem = ({ item, index }) => {
        ...
        return (
            <View style={{ flexDirection: 'row' }}>
                <View style={{ width: 70 }}>
                    <Text style={styles.textfont}>{actualIndex}</Text>
                </View>
    ...
    item['Dept value'] === 'Pediatrics' ? pratCurrent :
